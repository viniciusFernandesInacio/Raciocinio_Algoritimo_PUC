...
for i in range(len(matriz)):
    for j in range(len(matriz)):
        ...
        if j % 2 == 0:
            soma_par += matriz[i][j]
        if (i + j <= (len(matriz)-2)):
            soma_diagonalcima += matriz [i][j] 
        if (i + j >= (len(matriz))):
            soma_diagonalbaixo += matriz [i][j]              
print(f'Diagonal principal = {soma_dp}.\nDiagonal secundária = {soma_ds}.\nAcima da diagonal = {soma_da}.')
print(f'Abaixo da diagonal principal = {soma_db}.\nColunas pares = {soma_par}.\nSoma acima da diagonal secundária = {soma_diagonalcima}.\nSoma abaixo da diagonal secundária = {soma_diagonalbaixo}.')                     